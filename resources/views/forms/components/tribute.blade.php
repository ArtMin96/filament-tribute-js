...
>
    ...
    >
        <div x-data="tributeFormComponent({
                state: $wire.{{ $applyStateBindingModifiers('entangle(\'' . $getStatePath() . '\')') }},
                mentions: {
                    endpoint: @js($getEndpoint()),
                    mentionables: @js(json_decode($getMentionables())),
                    fillAttr: @js($getFillAttr),
                    selectClass: @js($getSelectClass()),
                    containerClass: @js($getContainerClass()),
                    itemClass: @js($getItemClass()),
                    allowSpaces: @js($getAllowSpaces()),
                    replaceTextSuffix: @js($getReplaceTextSuffix()),
                    requireLeadingSpace: @js($getRequireLeadingSpace()),
                    positionMenu: @js($getPositionMenu()),
                    spaceSelectsMatch: @js($getSpaceSelectsMatch()),
                    autocompleteMode: @js($getAutocompleteMode()),
                    menuItemLimit: @js($getMenuItemLimit()),
                    menuShowMinLength: @js($getMenuShowMinLength()),
                }
            })"
             wire:ignore
        ></div>
